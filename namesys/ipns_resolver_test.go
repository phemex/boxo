...
func TestResolver(t *testing.T) {
	...
	t.Run("Publish and resolve", func(t *testing.T) {
		...
		res, _, err := resolver.Resolve(context.Background(), name.String())
		require.NoError(t, err)
		require.Equal(t, pathCat, res)
	})
	...
	t.Run("Resolve does not return expired record", func(t *testing.T) {
		...
		_, _, err = resolver.Resolve(context.Background(), name.String())
